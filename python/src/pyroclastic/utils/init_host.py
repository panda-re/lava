...
def main():
    ...
    if not os.path.isfile(host_json_path):
        progress("Building host.json")
        # Build host.json
        json_configs = {
            "qemu": "panda-system-x86_64",
        ...
        progress(f"host.json created at {host_json_path}")
        with open(host_json_path, 'w+') as f:
            f.write(json.dumps(json_configs, indent=4))
    else:
        progress(f"Found existing host.json at {host_json_path}")

...
def analysis(project_name: str, output_file: str, inputs: list[str]):
    ...
    for filename in inputs:
        ...
        assert yaml_content is not None, "Missing output file from fninstr"
        for entry in yaml_content:
            if 'fun' in entry:
                fd = Function(entry['fun'])
                if fd.start == fd.end:
                    continue
                if fd.hasbody:
    ...
    for name in prots.keys():
        all_fns.add(name)
    print(f"{len(all_fns)} fn names in prots")
    for name in fundefs.keys():
        all_fns.add(name)
    print(f"{len(all_fns)} fn names in prots+fundefs")
    for name in calls.keys():
        all_fns.add(name)
    print(f"{len(all_fns)} fn names in prots+fundefs+calls")
    for name in calls.keys():
    ...
    1. Obviously, only if 'foo' has an implementation (body) can it be
       a candidate to be instrumented in the first place.
    2. Say a function 'foo' is a candidate for instrumentation. But
       'foo' is passed, as a parameter, to another function, 'bar'.
       If 'bar' is not a candidate for instrumentation then neither can
       'foo' be since calls to 'foo' from bar can't be instrumented.
       Note that resolving this sort of relation requires recursing.
    3. If a function's body isn't instrumented then calls to that function
       cannot be instrumented with data_flow arg.
    ...
    for name in all_fns:
        if name in fundefs:
            for fd in fundefs[name]:
                ...
                if debug:
                    print(f"Instr candidate {name} has body")
                break
        else:
            ...
            if debug:
                print(f"Won't instrument {name} (data_flow) since we don't have body")
    ...
    other fns that are, themselves, not instrumental.  Which means
    ...
    # Ok we have a list of instrumental functions.
    # Now, we need to transitively close.
    # If fn1 is un-instrumental, and if contains calls
            ...
            if instr_judgement[called_fn_name] is OKI:
                ...
                for call in calls[called_fn_name]:
                    if not (instr_judgement[call.containing_function] is OKI):
                        ...
                        any_change = True
                        print(f"Cant instrument {called_fn_name} because its called from {call.containing_function} which we can't instrument")
    ...
    for i in range(4):
        if i in ninstr:
            print(f"instrflags={i}: count={ninstr[i]}")

    for name in instr_judgement.keys():
        if instr_judgement[name] == OKI:
            print(f"Instrumenting fun [{name}]")

    banned_functions = project_config.get("df_fn_blacklist", [])

...
Here's what you need to do to lava something.

I assume your project is a configure / make / make install type beast.
If not, good luck.

1. Make sure to configure and make for a 32-bit target.  That is what
we generate for rode0day.  You want any conditionally compiled src
specific to 32-bit to be chosen.  Easiest way to ensure this is to
configure / make inside the lava32 container.

2. Obtain pre-processed versions of all the source required to compile
your target.  Here's how I I do this.  First, I run make (maybe V=1
make for verbose output) and collect the output to a file.  Maybe
something like

V=1 make >& make.out

Second, I grep that make.out for gcc and ar lines.  These I transform
with sed to turn them into new lines that, instead of compiling object
files, create preprocessed code.  All of these lines I collect into a
...
an unitialized variable as being attacker controlled when that happens probabilisitcally.
...
3.2. Simple makefile MUST compile with -g.  Otherwise PRI wont work
under panda and you'll never get any duas or attack points.

3.3. Simple makefile should compile with -O0.  This is very imporant
...
install.  Make install should create a lava-install dir and beneath
that a bin directory at least which is where the compiled program gets
installed.

4. The result of all of 1-3 should be a tar ball like the ones in 
target_bins.  

5. Now create a config json file for the target.  Crib from
target_config/file/file.json.  If your target is called foo, put it in
...
What might have gone wrong?  If you got NO bugs at this point, this is
probably bc of a problem with PRI in panda.  Make sure the program is
compiled with -g (so that dwarf info and symbols are in there).  Take
a look at the bug_mining output log and make sure the main executable
...
This probably won't work.  Sadly, even though everything through
bug_mining (panda pri taint analysis and postprocess with
find_bug_inj.cpp) works, you will likely still have problems with
injection.  This is bc injection is where we add code to implement the
...
defined function. That often fails b/c of fn pointers.

You have three options at this point.  

OPTION 1. You can try to fiddle with the Clang src-to-src transform to get
...
OPTION 2. You can sometimes make a problem go away by hacking the
...
OPTION 3. You can start writing sed rules to fix up the source so that it
...
src *after* inject.py to correct these kinds of bloomers.  Easier than changing lavaTool

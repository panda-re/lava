...
Because they ensure that attacker-controlled data is available to influence program behavior, DUAs form the raw material from which we construct bugs. To identify DUAs in program, we perform the following actions.

The program is executed under a dynamic taint analysis~\cite{newsometaint} for a specific input.
...
Comparing the input bytes one by one, for example, would allow coverage-maximizing fuzzers like American Fuzzy Lop (AFL) to incrementally discover the bug by guessing one byte at a time, rather than having to guess the entire 32-bit trigger at once.

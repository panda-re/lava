...
Multiple threads and processes are also handled correctly, so that taint is flows are not lost..
...
However, in order to ensure that the bug only manifest itself very occasionally (one of our requirements from Section~\ref{sec:motivation}), we  add a guard requiring that the DUA match a specific value if it is to be used to manifest the vulnerability.

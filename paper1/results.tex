...
The tools were selected because fuzzing and symbolic execution are extremely popular techniques for finding real-world bugs.
FUZZER and SES are both state-of-the-art, high-profile tools. 
For each tool, we expended significant effort to ensure that we were using tools correctly.
...
We did not contact tool authors or companies for guidance in tool use, as this seemed likely to bias results.
...
Each bug was inserted into a different copy of the source, and came with a fuzzed version of the input that was verified to trigger a crash.
...
\begin{description}
    ...
    \item [Range] These bugs trigger if the magic value is in some range, but also use the magic value to determine how much to overflow.
...
We chose four programs from the \verb+coreutils+ suite and injected as many verified bugs as possible.
Because the \verb+coreutils+ programs are quite small, and because we only used a single input file for each, the total number of bugs injected into each program was not very high. The exception to this is the \verb+who+ program, which reads a binary file with many unused fields, and therefore had many DUAs that could be used in bugs.

Our corpus was a collection of 69 modifications of the program \verb+file+.
Each modification represented one of six different bug types and a randomly selected $Bug ID$.
FUZZER was configured to use a single process and allowed to run for 1 hour on each version; the machine used for the test was a 64-bit Debian 7 server with 16 Intel Xeon cores at 3.50 GHz and 365 GB of \uppercase{ram}.
SES was allowed to run for five hours on each version, using a 64-bit Debian 8 server with 24 Intel Xeon cores at 3.47 GHz with 189 GB of \uppercase{RAM}.

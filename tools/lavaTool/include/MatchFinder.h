        ...
        if (LavaAction == LavaQueries) {
...
#endif
            {
                ...
                top << ""
                    ...
                    << "((!(__builtin_popcount((lava_extra[slot]>>16)&0x5aa5)&2)) && (__builtin_clz(lava_extra[slot])))\n"
                    //<< "__attribute__((visibility(\"default\")))\n"
                    << "#define lava_check_const_high_2(slot) "
                    // 1. (regview) << "((!(((lava_extra[slot])>>18)&1)) && (__builtin_popcount(lava_extra[slot]&0xff000000)<7))\n"
                    << "((__builtin_bswap32(lava_extra[slot])&3) && (__builtin_clz(lava_extra[slot]&0xdeadbeef)>3)) \n"
                    //<< "__attribute__((visibility(\"default\")))\n"
                    << "#define lava_check_const_high_3(slot) "
                    // 1. (regview) << "(__builtin_clz(lava_extra[slot])>4)\n"
                    << "(__builtin_parity((lava_extra[slot])>>16) && ((__builtin_ctz(__builtin_bswap32(lava_extra[slot])))>3))\n"
                    << "#define lava_check_const_high_4(slot) "
                    // 1. (regview) << "((((int)lava_extra[slot])>((int)-1)) && ((lava_extra[slot]>>16)&0x0101))\n"
                    << "(((((lava_extra[slot]>>16)*0xfe)&0xf0f0)==0xf0f0) && (__builtin_ffs(lava_extra[slot]>>16)>3))\n"

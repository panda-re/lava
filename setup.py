...
def main():
    ...
    if not os.path.isfile(os.path.join(LAVA_DIR, "src_clang/config.mak")):
        with open("src_clang/config.mak", "w") as f:
            f.write("# this is an autogenerated file based off of install location of LLVM\n")
            f.write("LLVM_SRC_PATH := {}\n".format(LLVM_DIR))
            f.write("LLVM_BUILD_PATH := {}\n".format(LLVM_DIR))
            f.write("LLVM_BIN_PATH := {}\n".format(os.path.join(LLVM_DIR, "Release/bin")))

    progress("Compiling fbi")
    if not os.path.isfile(os.path.join(LAVA_DIR, "fbi/fbi")):
        os.chdir(os.path.join(LAVA_DIR, "fbi"))
        with open("panda.mak", "w") as f:
